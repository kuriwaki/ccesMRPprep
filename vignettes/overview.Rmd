...
All CCES data can be downloaded directly from dataverse, using the development version of the [`dataverse`](https://github.com/IQSS/dataverse-client-r) R package. The function `get_cces_dataverse()` will make this quick and simple - you only need to specify the name of the dataset, given in `?cces_dv_ids`.
...
See `get_cces_question()` for how to get the _outcome_ data, which is a single column from another CCES dataset. This still relies on a flat file being pre-downloaded (via `get_cces_dataverse()`). 
...
Another way to draw the data would be to use `get_cces_dataverse()`, use the demographics from the cumulative passed into `ccc_std_demographics()`, and then merge the outcome from each year's CCES (e.g. `get_cces_dataverse("2016")`) using `get_cces_question()`. The year-specific CCES does come with demographics too, but they are not guaranteed to be standardized by this package's procedures. Use the formula, the standardized cumulative CCES, and a column of outcomes, and the ACS.
